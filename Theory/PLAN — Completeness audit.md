...
## Этап 4 — Senior-темы: тестирование, CI/CD, KMP, архитектурный собес — СДЕЛАНО
...
### 4.4 Архитектурный собес (senior) — СДЕЛАНО
- [x] `Patterns/Mobile app system design` — расширен: тайминг ответа,
      чеклист уточняющих вопросов (продуктовые / данные / ограничения /
      нефункциональные), таблица развилок и формула проговаривания
      trade-off «вариант → чем платим → почему здесь», типичные ошибки
      кандидата.
- [x] `Architecture interview. Типовые задачи` — новая заметка: лента
      с оффлайном (инвалидация, позиция скролла, оптимистичные действия),
      чат (локальный vs серверный id, порядок, дедупликация), загрузка
      файлов (WorkManager, chunked, Scoped Storage), карты (батарея,
      разрешения, кластеризация); универсальные краевые случаи.
- [x] `IDE/Multi-module architecture` проверена — уже learning-grade,
      не трогал.
- [x] Миграции и эволюция (View → Compose, один модуль → многомодульность)
      — раздел в `Mobile app system design`; feature flags и A/B там же
      в нефункциональном блоке.
