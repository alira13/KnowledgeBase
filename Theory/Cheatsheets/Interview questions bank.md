...
4. Почему `addAll` небезопасен в Java и при чём тут PECS? → сигнатура `Collection<? super T>` (consumer → super); без wildcard в коллекцию можно было бы положить неверный тип. См. [[Generics]].
